...
├── scenario.md        # required — human-readable repro steps (the "setlist")
...
expected: |
  ...
  reviewers see at the top of the generated README in the tape.
...
The YAML parser used by `warp-taper` is `awk`-grade. Keep it simple. Lists
inside lists are not supported. Comments are fine. Tilde expansion in paths
is handled by the runner.

## `scenario.md`

A markdown file describing the repro steps. The runner shows this to the
user before recording starts, and embeds it (blockquoted) in the tape's
generated `README.md`. Keep it focused on:
...
- Recording the *pre*-state of files the assertions will later compare to

The script's working directory is the scenario directory. It runs with
`set -euo pipefail` implied (the runner uses `bash setup.sh`).

## `assertions.sh`

Runs after recording finishes. The runner exports these env vars before
invoking it:
...
The script can source `lib/evaluate.sh` from `warp-taper` for helpers:
...
Each helper writes a one-line `  ✓ ...` or `  ✗ ...` to stdout. The bundle
extracts those lines for the README's assertion summary.

Exit code: `0` for "scenario verified," non-zero for "did not verify."

## Output bundle

After `warp-taper run scenarios/<scenario>/`, the tape directory contains:
...
├── master.mov                   # AUD: screen recording
├── patches/                     # named stills
│   ├── 01-before-rotation.png
│   └── 02-after-rotation.png
├── logs/
│   ├── warp-oss.session.log     # SBD: warp-oss.log slice during the session
...
## What scenarios are good for vs. what they aren't

**Good fit** — verifiable cross a deterministic boundary:

- File-existence checks (rotation files appear)
- Log-content greps (a specific WARN does/doesn't appear)
- Settings UI state (configurable via `defaults write` for some settings)
...
For the second category the value is the recording itself, plus the
human eye on it. Assertions can still be useful (e.g. "no crash logs
appeared in the SBD") but won't fully verify the visual claim.
