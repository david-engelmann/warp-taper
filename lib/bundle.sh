...
bundle_tape() {
    ...
    local readme="${tape_dir}/README.md"
    {
        ...
        if [[ -f "${tape_dir}/master.mov" ]]; then
            echo "- [master.mov](master.mov) — screen recording"
        fi
        ...
        if [[ -f "${tape_dir}/logs/warp-oss.session.log" ]]; then
            local session_bytes
            session_bytes="$(wc -c <"${tape_dir}/logs/warp-oss.session.log" | tr -d ' ')"
            echo "- [logs/warp-oss.session.log](logs/warp-oss.session.log) — ${session_bytes} bytes of warp-oss.log captured during the session"
        fi
        ...
        echo "---"
        echo
        echo "_Generated by [warp-taper](https://github.com/david-engelmann/warp-taper)._"
    } >"${readme}"
